...
class ResNetModel(object):
    ...
    def __init__(self,x, is_training,reuse=False,depth=50, dropout_rate=0.5,num_classes=1000):
    ...
    def inference(self):
        # Scale 1
        with tf.variable_scope('scale1',reuse=self.reuse):
            s1_conv = conv(self.input, ksize=7, stride=2, filters_out=64)
            s1_bn = bn(s1_conv, is_training=self.is_training)
            s1 = tf.nn.relu(s1_bn)

        # Scale 2
        with tf.variable_scope('scale2',reuse=self.reuse):
        ...
        ######### add adapted layer ###############
        with tf.variable_scope('adapt',reuse=self.reuse):
            # self.avg_pool=tf.nn.dropout(self.avg_pool,keep_prob=self.dropout_rate)
            self.adapt= tf.nn.tanh(fc(self.avg_pool, num_units_out=180))
            # self.adapt=tf.transpose(self.adapt)/tf.norm(self.adapt,axis=1)
            # self.adapt=tf.transpose(self.adapt)

        with tf.variable_scope('fc',reuse=self.reuse):
            self.prob = fc(self.adapt, num_units_out=self.num_classes)
...
def stack_Final(x, is_training, num_blocks, stack_stride, block_filters_internal):
    for n in range(num_blocks):
        block_stride = stack_stride if n == 0 else 1
        with tf.variable_scope('block%d' % (n + 1)):
            x = block_Final(x, is_training, block_filters_internal=block_filters_internal, block_stride=block_stride)
    return x

def block_Final(x, is_training, block_filters_internal, block_stride):
    filters_in = x.get_shape()[-1]
